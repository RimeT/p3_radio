...
class PyRadiomicsCommandLine:
  ...
  def __init__(self, custom_arguments=None):
    self.logger = logging.getLogger('radiomics_ria.script')  # holds logger for script events
  ...
  def getParser(cls):
    ...
    parser.add_argument('--version', action='version', help='Print version and exit',
						version='%(prog)s ' + radiomics_ria.__version__)
    return parser

  def run(self):
    # Run the extraction
    try:
      self.logger.info('Starting PyRadiomics (version: %s)', radiomics_ria.__version__)
    ...
    for case_idx, case, param, setting_overrides in case_generator:
      if case_idx == 1 and param is not None:
        if not os.path.isfile(param):
          ...
          schemaFile, schemaFuncs = radiomics_ria.getParameterValidationFiles()
  ...
  def _configureLogging(self):
    ...
    if self.args.log_file is not None:
      ...
      logging_config['loggers']['radiomics_ria']['handlers'].append('logfile')
