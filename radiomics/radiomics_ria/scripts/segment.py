...
import radiomics_ria.featureextractor

caseLogger = logging.getLogger('radiomics_ria.script')
...
def _extractFeatures(case_idx, case, config, config_override):
  ...
  try:
    ...
    extractor = radiomics_ria.featureextractor.RadiomicsFeaturesExtractor(config, **config_override)
...
def _configureParallelExtraction(logging_config, add_info_filter=True):
  ...
  if add_info_filter:
    ...
    # Adding the filter to the first handler of the radiomics_ria logger limits the info messages on the output to just
    # those from radiomics_ria.script, but warnings and errors from the entire library are also printed to the output.
    # This does not affect the amount of logging stored in the log file.
    outputhandler = radiomics_ria.logger.handlers[0]  # Handler printing to the output
    outputhandler.addFilter(info_filter('radiomics_ria.script'))
  ...
  radiomics_ria.logger.debug('parallel extraction configured')
