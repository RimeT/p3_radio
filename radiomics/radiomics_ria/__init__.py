...
def setVerbosity(level):
  ...
  By default, the radiomics_ria logger is set to level "INFO" and the stderr handler to level "WARNING". Therefore a log
  storing the extraction log messages from level "INFO" and up can be easily set up by adding an appropriate handler to
  the radiomics_ria logger, while the output to stderr will still only contain warnings and errors.

  .. note::

    This function assumes the handler added to the radiomics_ria logger at initialization of the toolbox is not removed from
...
def getFeatureClasses():
  """
  Iterates over all modules of the radiomics_ria package using pkgutil and subsequently imports those modules.
  ...
  with name starting with 'Radiomics' and is inherited from :py:class:`radiomics_ria.base.RadiomicsFeaturesBase`.
  ...
  if _featureClasses is None:  # On first call, enumerate possible feature classes and import PyRadiomics modules
    ...
    for _, mod, _ in pkgutil.iter_modules([os.path.dirname(__file__)]):
      if str(mod).startswith('_'):  # Skip loading of 'private' classes, these don't contain feature classes
        continue
      __import__('radiomics_ria.' + mod)
      module = sys.modules['radiomics_ria.' + mod]
...
def getImageTypes():
  ...
  in :ref:`imageoperations <radiomics_ria-imageoperations-label>`. Returns a list containing available image type names
...
# 1. Set up logging
debugging = True
logger = logging.getLogger(__name__)  # 'radiomics_ria'
...
cShape = None
try:
  from radiomics_ria import _cmatrices as cMatrices  # noqa: F401
  from radiomics_ria import _cshape as cShape  # noqa: F401
