...
from radiomics_ria import generalinfo, getFeatureClasses, getImageTypes, getParameterValidationFiles, \
	imageoperations

bin_counts = [2, 4, 8, 32, 64]
distances = [1, 2, 3]

shape_bin_counts = [1] + bin_counts

class RadiomicsFeaturesExtractor:
	r"""
    Wrapper class for calculation of a radiomics_ria signature.
    ...
    Then a call to :py:func:`execute` generates the radiomics_ria
    signature specified by these settings for the passed image and labelmap combination. This function can be called
    repeatedly in a batch process to calculate the radiomics_ria signature for all image and labelmap combinations.
    ...
    :ref:`Customizing the Extraction <radiomics_ria-customization-label>`.
	...
	def _applyParams(self, paramsFile=None, paramsDict=None):
		...
		if len(pykwalify.core.log.handlers) == 0 and len(logging.getLogger().handlers) == 0:
			# No handler available for either pykwalify or root logger, provide first radiomics_ria handler (outputs to stderr)
			pykwalify.core.log.addHandler(logging.getLogger('radiomics_ria').handlers[0])
	...
	def enableImageTypeByName(self, imageType, enabled=True, customArgs=None):
        ...
        - Wavelet: Wavelet filtering, yields 8 decompositions per level (all possible combinations of applying either
          ...
          See also :py:func:`~radiomics_ria.imageoperations.getWaveletImage`
        - LoG: Laplacian of Gaussian filter, edge enhancement filter. Emphasizes areas of gray level change, where sigma
          ...
          See also :py:func:`~radiomics_ria.imageoperations.getLoGImage`
        ...
        :ref:`imageoperations<radiomics_ria-imageoperations-label>`
        (:py:func:`~radiomics_ria.imageoperations.getSquareImage`,
        :py:func:`~radiomics_ria.imageoperations.getSquareRootImage`,
        :py:func:`~radiomics_ria.imageoperations.getLogarithmImage` and
        :py:func:`~radiomics_ria.imageoperations.getExponentialImage`,
        respectively).
        """
	...
	def enableAllFeatures(self):
        ...
        .. note::
          ...
          manually by a call to :py:func:`~radiomics_ria.base.RadiomicsBase.enableFeatureByName()`,
	...
	def execute(self, imageFilepath, maskFilepath, label=None, label_channel=None, voxelBased=False):
		"""
        Compute radiomics_ria signature for provide image and mask combination. It comprises of the following steps:
		...
		if not voxelBased:
			...
			if 'shape' in self._enabledFeatures.keys():
				if Nd == 3:
					...
					self.logger.warning(
						'Shape features are only available 3D input (for 2D input, use shape2D). Found %iD input', Nd)
	...
	def computeFeature(self, featureClassName, image, mask, kwargs, bin_count, distance):
		featureVector = collections.OrderedDict()
		...
		for feature, enabled in six.iteritems(featureClass.enabledFeatures):
			feature_function = getattr(featureClass, 'get%sFeatureValue' % feature)
			feature_args = inspect.getargspec(feature_function).args
			chosen_args = []
			chosen_args_name = []
			if "suffix" in feature_args:
				chosen_args += [["origin", "sq", "en"]]
				chosen_args_name += ["suffix"]
			if "nd" in feature_args:
				chosen_args += [[True, False]]
				chosen_args_name += ["nd"]
			if "power" in feature_args:
				if "cluster" in feature.lower():
					chosen_args += [[1, 2, 3, 4]]
				else:
					chosen_args += [[1, 2]]
				chosen_args_name += ["power"]
			if "inverse" in feature_args:
				chosen_args += [[False, True]]
				chosen_args_name += ["inverse"]
			if "percent" in feature_args:
				if "deciles" in feature.lower():
					chosen_args += [range(10, 100, 10)]
				else:
					chosen_args += [[0.025, 0.05, 0.10, 0.25]]
				chosen_args_name += ["percent"]

			try_args = list(product(*chosen_args))
			for args in try_args:
				feature_kwargs = dict()
				for i in range(len(chosen_args_name)):
					feature_kwargs[chosen_args_name[i]] = args[i]
				featureValue = feature_function(**feature_kwargs)
				name_list = [None]*(len(chosen_args_name)+len(chosen_args))
				name_list[::2] = chosen_args_name
				name_list[1::2] = args
				prefix = [featureClassName, feature]
				if bin_count is not None:
					prefix += ["b", str(bin_count)]
				if distance is not None:
					prefix += ["d", str(distance)]
				name_list = prefix + name_list
				featureName = "_".join([str(x) for x in name_list])
				featureName = featureName.replace("suffix", "f")
				featureName = featureName.replace("True", "1")
				featureName = featureName.replace("False", "0")
				featureName = featureName.replace("percent", "p")
				featureName = featureName.replace("inverse", "inv")
				featureClass.featureValues[featureName] = np.squeeze(featureValue)
				featureVector[featureName] = featureValue
		return featureVector
	...
	def computeFeatures(self, image, mask, imageTypeName, **kwargs):
		...
		for featureClassName, enabledFeatures in six.iteritems(self._enabledFeatures):
			...
			if featureClassName in self.getFeatureClassNames():
				...
				if "first" in featureClassName:
					featureVector.update(self.computeFeature(featureClassName, image, mask, kwargs, None, None))
				else:
					for bin_count in bin_counts:
						kwargs['binCount'] = bin_count
						if "glcm" in featureClassName:
							for distance in distances:
								kwargs['distances'] = [distance]
								featureVector.update(self.computeFeature(featureClassName, image, mask, kwargs, bin_count, distance))
						else:
							featureVector.update(self.computeFeature(featureClassName, image, mask, kwargs, bin_count, None))
	...
	def computeShape(self, image, mask, boundingBox, shape_type='shape'):
		...
		for bin_count in shape_bin_counts:
			self.settings['binCount'] = bin_count
			featureClass = self.featureClasses[shape_type](croppedImage, croppedMask, **self.settings)
			featureClass.enableAllFeatures()
			featureClass._initCalculation()

			# for focus in featureClass.coefficients["grayLevels"]:
			for focus in range(1, featureClass.coefficients["Ng"] + 1):
				featureClass.update_focus(focus=focus)
				for feature, enabled in six.iteritems(featureClass.enabledFeatures):
					feature_function = getattr(featureClass, 'get%sFeatureValue' % feature)
					feature_args = inspect.getargspec(feature_function).args
					chosen_args = []
					...
					for args in try_args:
						...
						name_list[::2] = chosen_args_name
						name_list[1::2] = args
						name_list = ["shape", feature, "b", str(bin_count), "l", str(focus)] + name_list
						featureName = "_".join([str(x) for x in name_list])
						featureName = featureName.replace("True", "1")
						featureName = featureName.replace("False", "0")
						featureClass.featureValues[featureName] = np.squeeze(featureValue)
