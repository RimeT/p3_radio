...
def extract_feature(case):
    ...
    try:
        ...
        mask_arr = sitk.GetArrayFromImage(mask).astype(np.int)

        # 如果mask有很少的voxel，则略过
        voxels = np.sum(mask_arr)
        if voxels <= 10:
            delta_t = datetime.now() - t
            logging.getLogger('radiomics.batch').error('Case: %s %s %s processed in %s PID %s (%s)',
        ...
        logging.getLogger('radiomics.batch').error('Feature extraction failed!', exc_info=True)
