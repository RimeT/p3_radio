...
# label encoding
label_encoder, encoded_label, _ = tools.encode_b(tv_label + test_label) if not is_binary else tools.encode_l(tv_label + test_label)
    ...
    if 'svm' in m:
        ...
        if auto_opt:
            ...
            clf_probe = RandomizedSearchCV(svm.SVC(), param, n_iter=200, scoring='accuracy', verbose=1,
                                           cv=StratifiedKFold(n_splits=3, random_state=42), n_jobs=6, random_state=42)
        ...
        if auto_opt:
            ...
            clf_probe = GridSearchCV(LogisticRegression(tol=1e-6, max_iter=1000), param, scoring='accuracy', verbose=1,
                                     cv=StratifiedKFold(n_splits=3, random_state=42), n_jobs=6)
            clf_probe.fit(tv_feature, tv_label)
            return LogisticRegression(tol=1e-6, max_iter=1000, random_state=random_state, **clf_probe.best_params_), class_name
        else:
            return LogisticRegression(tol=1e-6, max_iter=1000, random_state=random_state, solver='liblinear'), class_name
    elif 'decision' in m:
        ...
        if auto_opt:
            ...
            clf_probe = RandomizedSearchCV(DecisionTreeClassifier(), param, n_iter=200, scoring='accuracy', verbose=2,
                                           cv=StratifiedKFold(n_splits=3, random_state=42), n_jobs=6)
            clf_probe.fit(tv_feature, tv_label)
        ...
        if auto_opt:
            ...
            clf_probe = RandomizedSearchCV(RandomForestClassifier(), param, n_iter=50, scoring='accuracy', verbose=2,
                                           cv=StratifiedKFold(n_splits=3, random_state=42), n_jobs=6)
            clf_probe.fit(tv_feature, tv_label)
        ...
        if is_binary:
            return xgb.XGBClassifier(objective='binary:logistic', n_estimators=200, max_depth=10, random_state=random_state), class_name
        else:
            return xgb.XGBClassifier(objective='multi:softprob', n_estimators=200, max_depth=200, random_state=random_state, num_class=class_nb), class_name
    elif 'deep' in m:
        ...
        if auto_opt:
            ...
            clf_probe = GridSearchCV(MLPClassifier(max_iter=5000, tol=1e-5), param, scoring='accuracy', verbose=1,
                                     cv=StratifiedKFold(n_splits=3, random_state=42), n_jobs=6)
            clf_probe.fit(tv_feature, tv_label)
            return MLPClassifier(random_state=random_state, max_iter=5000, tol=1e-5, **clf_probe.best_params_), class_name
        else:
            return MLPClassifier(random_state=random_state, max_iter=5000, tol=1e-5, learning_rate='adaptive'), class_name
    return None, None
...
def get_predict_report(clf, x, y, sess="train"):
    ...
    for i in used_origin_idex:
        ...
        roc[origin_label]['fpr'] = list(fpr[origin_label])
        roc[origin_label]['tpr'] = list(tpr[origin_label])
        roc[origin_label]['thresh'] = list(map(np.float64, list(thresh[origin_label])))
        roc[origin_label]['auc'] = auc_score
...
def classification_train(clf, clf_name):
    global summary_results, tv_feature
    cv_res = dict()
...
def learn():
    ...
    for m in models:
        ...
        classification_train(clf, clf_name)
